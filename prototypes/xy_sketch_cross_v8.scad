...
// V8: user's dimensioned sketch, 2026-09-06. Units: mm.
    ...
    for (x=[window_start,window_start+window_length])
        for (y=[y0,y0+window_width])
            translate([x-window_corner_reach,y-window_corner_reach])
                square([2*window_corner_reach,2*window_corner_reach]);
}
...
// Recess only one adjacent face at each corner. The other remains a flat
// nominal target, extended into the notch. The eps overlap is outside the
// original body: the removed rectangular footprint is exactly width x depth.
module recess(p, angle=0) {
    translate(p) rotate(angle)
        translate([0,-eps]) square([relief_width,relief_depth+eps]);
}
...
module gauge() {
    difference() {
        body();
        if (reliefs_enabled)
            translate([0,0,-0.1]) linear_extrude(height=height+0.2) corner_reliefs();
        if (windows_enabled && window_reliefs_enabled)
            translate([0,0,-0.1]) linear_extrude(height=height+0.2) window_reliefs();
        if (labels_enabled) {
